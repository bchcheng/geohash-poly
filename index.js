		...
		preparePoly(function(err, prepared) {
			...
			while (columnHash !== westerly) {
				if (self.hashMode === 'inside' && inside(columnCenter, prepared)) {
					...
					var bbox = self.geohashDecodeBbox(columnHash, self.precision);
					var nw = {
						latitude: bbox[2],
						longitude: bbox[1]
					};
					...
					if (inside(se, prepared) || inside(nw, prepared) || inside(sw, prepared) || inside(ne, prepared)) {
						rowHashes.push(columnHash);
					}
				}
				else if (self.hashMode === 'extent') {
					rowHashes.push(columnHash);
				}
			...
			if (self.hashMode === 'inside' || self.hashMode === 'extent' || self.hashMode === "intersect" || !rowHashes.length) {
				done(null, rowHashes);
			}
			else if (self.hashMode === 'intersect') {
